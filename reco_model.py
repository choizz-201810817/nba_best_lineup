...
# %%
def fix( df, team = None, position=None,season=2023):
...
# min(origin_df['pev'])
# %%
# 현 시즌 모든 팀에서의 각 포지션별 pev 평균값 구하기
def pos_pev(df, season=2023):
    
    position = ['PF', 'SG', 'PG', 'SF', 'C']
    pev = []
    for pos in position : 
        score = df[(df['position']==pos)&(df['season']==season)]['pev'].mean()
...
# 포지션별 추천 선수
def reco(df,team,position,th=0.98,season=2023):
    # scale_df = scale(df)
    bad_find = bad_pev(df, team, season)
    player = bad_find[2][bad_find[0].index(position)][0] # 특정되는 한 명의 선수
    seq = indices(df, team, player)

    pos_df = pos_stat(df, position)
    ...
    player_indices = [i[0] for i in sim_scores if i[1]>=th] # 유사도가 특정 임계치 이상이면 가져옴
...
player_indices = [i[0] for i in sim_scores if i[1]>0.95] # 유사도가 특정 임계치 이상이면 가져옴
# player_indices = [i for i in player_indices if origin_df['position'][i]=='PF'] #해당포지션으로만 추리기
# player_indices = [i for i in player_indices if origin_df['position'][i]!='Boston Celtics'] #다른팀 선수들로만 추리기
# player_indices = [i for i in player_indices if origin_df['season'][i]==2023] #현재시즌 선수들로만 추리기
len(player_indices)
# %%
sim_scores
